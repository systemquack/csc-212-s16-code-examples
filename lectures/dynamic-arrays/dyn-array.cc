...
// both constructors assume that cap is greater than zero
...
void DynArray::append(ul_int elem) {
    // check if needs to grow the array
    if (size == capacity) {
        n_resizes ++;
        // save pointer to array
        ul_int *old = data;
        // calculate new capacity
        ul_int increase = (ul_int) capacity * rate;
        capacity += (increase > 0) ? increase : 1;
        // allocate space for new array
        data = new ul_int [capacity];
        // copy all elements
